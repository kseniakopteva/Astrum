<x-profile-layout :user="$user" :followers="$followers" :following="$following">
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 xl:gap-10 max-w-5xl mx-auto">
        <div class="lg:col-span-2 bg-white dark:bg-neutral-800 p-4 pt-4 mb-4 break-inside-avoid border-l-4
        @if (!is_null($user->colour))
        border-{{ $user->colour->lightcolor }} dark:border-{{ $user->colour->darkcolor }} @else border-lime-600 dark:border-lime-700 @endif">
        ...
        <div class="bg-white 200 dark:bg-neutral-800 p-4 pt-4 mb-4 break-inside-avoid border-l-4
        @if (!is_null($user->colour))
        border-{{ $user->colour->lightcolor }} dark:border-{{ $user->colour->darkcolor }} @else border-lime-600 dark:border-lime-700 @endif">
            ...
            <div class="space-y-2">
                ...
                <div class="flex justify-between w-full items-center">
                    ...
                    <form method="POST" action="{{ route('about.link.destroy') }}">
                        ...
                        <button class="text-sm text-neutral-500" onclick="return confirm('Are you sure you want to remove this link?')">Remove this link</button>
                    </form>
                </div>
                @endforeach
            </div>
