    ...
    <form method="post" action="{{ route('profile.update') }}" class="space-y-6" enctype="multipart/form-data">
        <div class="md:grid md:grid-cols-4 md:gap-4">
            <div class="space-y-6 col-span-2">
                ...
                <div>
                    ...
                    <x-textarea id="bio" name="bio" type="text" class="mt-1 block w-full" rows="7" autofocus>{{ old('bio', $user->bio) }}</x-textarea>
            ...
            <div class="col-span-2 grid grid-cols-2 gap-6 w-full">
                          ...
                          focus:ring-lime-500 dark:focus:ring-lime-600 dark:ring-offset-neutral-800
                           focus:ring-2 dark:bg-neutral-700 dark:border-neutral-600"
                                    <?php if ($badge->id === $user->badge->id) {
                                        echo 'checked';
                                    } ?> type="radio" name="badge_id" id="{{ $badge->id }}" value="{{ $badge->id }}">
                                <label for="{{ $badge->id }}"
                ...
                <div class="space-y-4 mt-6 m-auto
                        ...
                        <div class="h-40 w-40 md:h-40 md:w-40 lg:h-52 lg:w-52 xl:h-60 xl:w-60" x-data="previewImage()">
                            <label for="image" class="relative cursor-pointer">
                                <div
                                    ...
                                    <img id="imagePreview" x-show="imageUrl" :src="imageUrl" class="object-cover w-full h-full">
                                    <div id="imagePlaceholder" x-show="!imageUrl" class="text-neutral-300 flex flex-col items-center w-full h-full"
                                        style="background-image: url('{{ asset('images/profile-pictures/' . auth()->user()->image) }}'); background-size: cover; background-position: center center;">
                                    </div>
        ...
        <div class="flex items-center gap-4 justify-end">
            @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="text-sm text-neutral-600 dark:text-neutral-400">{{ __('Saved.') }}</p>
            @endif
