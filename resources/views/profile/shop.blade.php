<x-profile-layout :user="$user" :followers="$followers" :following="$following">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
        ...
        @php if ($product->type === 'unlimited') $buy = 'Get'; else $buy = 'Buy'; @endphp
        <div class="rounded-md bg-white dark:bg-neutral-800 relative">
            <img class="cursor-pointer rounded-t-md h-56 w-full object-cover" src="{{ asset('storage/images/products/'.$product->image) }}" alt="" x-data="" x-on:click.prevent="$dispatch('open-modal', 'image-{{ $product->id }}')">
        ...
        <x-modal name="image-{{ $product->id }}" focusable maxWidth='5xl'>
            <div class="p-4">
                <img class="rounded-t-md w-full object-cover" src="{{ asset('storage/images/products/'.$product->image) }}" alt="">
            </div>
        ...
        $errors->getBag('default')->has('details'))
        ...
        <x-modal name="buy-product-{{ $product->id }}" focusable :show="$show">
            <div class="p-6">
                <h2 class="text-lg font-medium dark:text-neutral-100 mb-4">
                    {{ $buy }} <span class="font-bold">{{ $product->name }}</span> by <a href="{{ route('profile.index', $product->author->username) }}"><span class="font-bold">{{ $product->author->username }}</span></a>
                </h2>
