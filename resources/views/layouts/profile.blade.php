        ...
        <header class="mb-10 p-8 pb-4 bg-white backdrop-blur-md bg-opacity-70 dark:bg-opacity-90 dark:bg-neutral-800 rounded-b-md {{-- bg-white/50 dark:bg-neutral-800/50 backdrop-blur-sm --}}  max-w-4xl m-auto ">
            <div class="grid grid-cols-5">
                ...
                <div class=" col-span-2 sm:col-span-1 order-1 sm:order-1 profile-image-container mr-10 w-32 h-32 sm:w-full sm:h-auto self-start">
                    <img class="profile-image rounded-full shadow-md w-full h-auto" src="{{ asset('images/profile-pictures/' . $user->image) }}" alt="" class="profile-picture" width="100"
                        height="100" style=" pointer-events: none; user-select: none;">
                ...
                <section class="order-2 sm:order-3 mt-4 sm:mt-0 col-span-3 sm:col-span-1 flex flex-col
                            ...
                            @if ($user->id !== auth()->user()->id)
                                <div class="flex">
                                    @if (!$user->isBlockedBy(auth()->user()))
                                        ...
                                        @endif
                                    @else
                                        <button class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest"
                                            type="button" disabled>Blocked</button>
                                    @endif
                                    ...
                                    <x-dropdown align="right" width="52">
                                        ...
                                        <x-slot name="content">
                                            <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'report')"
                                            ...
                                            @if (!$user->isBlockedBy(auth()->user()))
                                                <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'block')"
                                                    class="block w-full px-4 py-2 text-left text-sm hover:bg-neutral-100 dark:hover:bg-neutral-800 focus:outline-none focus:bg-neutral-100 dark:focus:bg-neutral-800 transition duration-150 ease-in-out text-red-400 hover:text-red-600">
                                                    {{ __('Block') }}
                                                </button>
                                            @else
                                                <form method="POST" action="{{ route('unblock') }}">
                                                    ...
                                                    <button type="submit" onclick="return confirm('Are you sure you want to unblock this user?')"
                                            ...
                                            @if (in_array(auth()->user()->role, ['mod', 'admin']))
                                                ...
                                                @else
                                                    <form method="POST" action="{{ route('unban') }}">
                                                        @csrf
                                                        <input type="hidden" name="user_id" value="{{ $user->id }}">
                                                        <button type="submit" onclick="return confirm('Are you sure you want to unban this user?')"
                                                            class="block w-full px-4 py-2 text-left text-sm hover:bg-neutral-100 dark:hover:bg-neutral-800 focus:outline-none focus:bg-neutral-100 dark:focus:bg-neutral-800 transition duration-150 ease-in-out">
                                                            Unban</button>
                                                    </form>
                                                @endif
                    ...
                    <div class="flex space-x-4 items-start whitespace-nowrap justify-end mt-2">
                        <button @if ($user->followers->count() == 0) style="pointer-events: none" @endif class="hover:text-lime-700 flex flex-col items-center" x-data=""
                        ...
                        <button @if ($user->following->count() == 0) style="pointer-events: none" @endif class="hover:text-lime-700 flex flex-col items-center" x-data=""
