        ...
        <div class="mt-2 grid grid-cols-4 p-6">
            ...
            <div class="text-right flex flex-col justify-between mt-2">
                ...
                <div>
                                ...
                                @else
                                    <form action="{{ route('starshop.' . $type . 's.buy') }}" method="POST">
                                        ...
                                        <x-secondary-button type="submit">Buy another
                                            ({{ auth()->user()->postFrameAmount($item->id) }})
                                        </x-secondary-button>
                                    </form>
                                @endif
                            @endif
                    ...
                    @if ($type == 'post-frame' && auth()->user()->role == 'user')
                        <span class="text-xs text-red-500">You need to be a creator to use post frames.</span>
                    @endif
                </div>
            </div>
