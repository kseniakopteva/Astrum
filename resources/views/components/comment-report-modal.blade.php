...
<x-modal name="{{ $name }}" focusable>
    <div class="p-6">
        <h2 class="text-lg font-medium text-red-500 dark:text-neutral-100 mb-4">
            Report Comment <span class="font-bold">[{{ implode(' ', array_slice(explode(' ', $comment->body), 0, 5)) }}...]</span>
            by <a href="{{ route('profile.index', $comment->author->username) }}"><span class="font-bold">{{ $comment->author->username }}</span></a>
        </h2>
