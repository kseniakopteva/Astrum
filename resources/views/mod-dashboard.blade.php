                ...
                <div class="space-y-2">
                    @foreach ($reported_arr as $reported_type)
                        <div x-data="{ open: false }" class="w-full bg-neutral-50 dark:bg-neutral-900/50 rounded-md @if ($reported_type['arr']->where('resolved', 0)->count() > 0) shadow-md @endif">
                            <button @click="open=!open" <?php
                            if ($reported_type['arr']->where('resolved', 0)->count() <= 0) {
                                echo ' disabled ';
                            }
                            
                            $t = $reported_type['type'];
                            $count = \App\Models\Report::where('reported_type', $t)
                                ->where('resolved', 0)
                                ->count();
                            ...
                            if ($count > 0) {
                                ...
                                class="py-2 px-3 flex justify-between w-full @if ($count > 0) text-black dark:text-white hover:text-lime-600 @else text-neutral-400 dark:text-neutral-700 @endif text-lg"></button>


                            @if ($reported_type['arr']->count() > 0)
                                <div x-show="open" x-cloak class="mx-4 py-4" x-transition>
                                    <x-dashboard-section cols="2" :reported_arr="$reported_type['arr']" :type="$reported_type['type']" />
                                </div>
                            @endif
                        </div>
                    @endforeach
