<x-main-layout>
    <div class="wrapper mx-auto">
        ...
        <h1 class="medium-title">Search results for <strong
                class="font-bold">"{{ request(['search'])['search'] }}"</strong></h1>
        ...
        @if (count($users) !== 0)
            ...
            <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                @foreach ($users as $user)
                    <div
                        class="flex flex-col items-center border bg-white border-neutral-200 dark:bg-neutral-800 dark:border-neutral-700 rounded-lg p-4 pt-4 mb-4 break-inside-avoid">
                        <a href="{{ route('profile.index', $user->username) }}">
                            <img src="{{ asset('storage/images/profile-pictures/' . $user->image) }}" alt=""
                                class="rounded-full" width="130" height="130"></a>
                        <p class="text-lg mt-2">{{ $user->name }}</p>
                        <x-colored-username-link size="small"
                            :user="$user">{{ $user->username }}</x-colored-username-link>
                    </div>
                @endforeach
            </div>
        @endif
