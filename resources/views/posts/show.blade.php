<x-main-layout>
        ...
        <article
            ...
            <div class="lg:border-s border-neutral-300 dark:border-neutral-700 ps-4 flex flex-col">
                <div class="flex justify-between items-baseline mt-4 lg:mt-0">
                    ...
                    @if (auth()->check() && auth()->user()->id == $post->author->id && $post->author->isBanned($post->author)
                    || auth()->check() && !$post->author->isBanned(auth()->user())
                    )
    ...
    <section class="space-y-4 max-w-3xl m-auto">
        ...
        @if (!$post->author->isBanned($post->author))
            @auth
                            ...
                            <x-dropdown align="right" width="52">
                                ...
                                <x-slot name="content">
                                    @if (auth()->check() && $comment->author->id === auth()->user()->id)
                                        <form
                                            action="{{ route('post.comment.delete', ['comment' => $comment->id, 'post' => $comment->post->id]) }}"
                                            method="POST">
                                            @csrf
                                    ...
                                    @elseif (auth()->check() && $comment->author->id !== auth()->user()->id)
                                        <button x-data=""
                                            x-on:click.prevent="$dispatch('open-modal', 'comment-report-{{ $comment->id }}')"
                                            class="block w-full px-4 py-2 text-left text-sm hover:bg-neutral-100 dark:hover:bg-neutral-800 focus:outline-none focus:bg-neutral-100 dark:focus:bg-neutral-800 transition duration-150 ease-in-out text-red-400 hover:text-red-600">
                                            {{ __('Report') }}
                                        </button>
                                    @endif
